...
const Wrapper = ({ searchTerm }: { searchTerm: string }): ReactElement => {
  const addressBook = {
  ...
  return (
    <AddressBookLocal onAddressSelect={() => {}} searchTerm={searchTerm} addressBook={addressBook} network="local" />
  );
};

describe("addressBookLocal", () => {
  it("should show default message", () => {
    render(<AddressBookLocal onAddressSelect={() => {}} searchTerm={""} addressBook={{}} network="local" />);
