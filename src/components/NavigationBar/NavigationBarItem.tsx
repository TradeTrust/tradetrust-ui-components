...
const NavigationLink: FunctionComponent<{ item: NavType.NavigationLink }> = ({ item }) => {
  return (
    <a
      className={`text-pale-sky-grey-500 hover:text-black transition-color duration-200 ease-out font-medium ${item.className}`}
      href={item.path}
    >
      {item.label}
    </a>
...
const IconButton: FunctionComponent<{ item: NavType.IconButton }> = ({ item }) => {
  ...
  return (
    <a
      className={`text-pale-sky-grey-500 hover:text-black transition-color duration-200 ease-out  ${item.className}`}
      href={item.path}
      data-testid={item.id}
    >
  ...
  return (
    <div className="relative">
      <button
        ...
        id={item.id + "-button"}
      >
        <span className="text-pale-sky-grey-500 hover:text-black transition-color duration-200 ease-out font-medium">
          {item.label}
        </span>
        <svg
          className={`-mr-1 ml-2 h-5 w-5 transition-transform duration-200 ease-out transform text-pale-sky-grey-500 ${
            ...
            <div className="dropdown py-1">
              {item.dropdownItems?.map((dropdownItem: any, index: number) => {
                return (
                  <a
                    key={index}
                    className="text-pale-sky-grey-500 hover:text-black transition-color duration-200 ease-out font-medium block px-4 py-2"
